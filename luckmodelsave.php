...
if($_GET["act"] =="modelsave"){
	$tzmodel_array=array();
	$query=$db->query("Select tznum,tzpoints,points from {$web_dbtop}game28_users_tz where NO=".intval($_GET["luckno"])." and uid=".intval($_COOKIE["usersid"])." Order by id desc");
	...
	if($_POST["tbID"]){
		$db->query("update {$web_dbtop}game28_auto_tz set tzname='".str_check($_POST["tbModelName"])."',tzunm='".$tznum."',tzpoints=$tzpoints where id =".intval($_POST["tbID"]));
	}else{
		$db->query("INSERT INTO {$web_dbtop}game28_auto_tz (uid,tzname,tzunm,tzpoints,tzid) VALUES (".intval($_COOKIE["usersid"]).",'".str_check($_POST["tbModelName"])."','".$tznum."',$tzpoints,".intval($_POST["tbModelID"]).")");
	}
	echo "<script language=javascript>alert('投注模式保存成功！');location.href='luckmodelsave.php?luckno=".$_GET["luckno"]."';</script>";
	exit;
}
?>
		...
		<DIV class="webgame-info">
			<DIV class="webgame-info-tabs">
			  ...
			  <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
                <tr>
                  <td bgcolor="#248301"><form action="?act=modelsave&luckno=<?=$_GET["luckno"]?>" method="post">
			  ...
			  <?
				...
				while($rs=$db->fetch_array($query)){
					$game28tz_array[]="{\"ID\":\"".$rs["id"]."\",\"modelID\":\"".$rs["tzid"]."\",\"modelName\":\"".$rs["tzname"]."\"}";
				}
