...
$db->query("insert into {$web_dbtop}deposit(uid,amnt,pointsget,time,upperid,upperget,operation) values({$userid},'{$amnt}','{$pointsget}','{$time}',{$upperid},'{$upperget}','{$operation}')");   //执行sql的写法  这句是例子 
...
//接下来是否要发送站内信 给上线？？？？？？？？？？？？？？？？？
$db->query("INSERT INTO {$web_dbtop}msg (usersid,title,mag,mid,time) VALUES (0,'下线充值奖励','您推荐的用户".$userid."充值$".$amnt.",奖励您游戏币".$upperget."',".intval($upperid).",'".date("Y-m-d H:i:s")."')");
