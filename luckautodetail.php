...
$query=$db->query("Select * from {$web_dbtop}game28_auto where uid=".intval($_COOKIE["usersid"])." Order by id desc");
if($rs=$db->fetch_array($query)){
	...
	echo "<script language=javascript>alert('sorry£¡what are you doing?');location.href='luck.php';</script>";
	exit;
}
$query=$db->query("Select * from {$web_dbtop}game28_auto_tz where id=".$autoid." Order by id asc");
if($rs=$db->fetch_array($query)){
	$tzname=$rs["tzname"];
...
if($_GET["act"] =="endStart"){
	$db->query("delete from {$web_dbtop}game28_auto where id=".$id);
	echo "<script language=javascript>alert('Autopress cancelled');location.href='luckautoset.php';</script>";
	exit;
}
?>
	...
	<DIV class="page_content">
		...
		<DIV class="webgame-info">
            ...
            <tr>
              ...
              <td background="images/game2_04.jpg">Autopress options£º</td>
            ...
            <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
              <tr>
                <td bgcolor="#248301"><table width="100%" border="0" cellpadding="0" cellspacing="1">
                    <tr>
                      <td colspan="4" bgcolor="#FFFFFF" class="font12">&nbsp;&nbsp;From Draw#<font class="rbf"><?=$startNO?></font> to <font class="rbf">Draw#<?=$endNO?></font><br />
                      &nbsp;&nbsp;Auto terminate when <?=$web_moneyname?> < <font class="rbf"><?=$minG?></font> or <?=$web_moneyname?> > <font class="rbf"><?=$maxG?></font><br />
                      &nbsp;&nbsp;Current pattern: <font class="rbf"><?=$tzname?></font>£¬<font class="rbf"><?=$tzpoints?></font> <?=$web_moneyname?> per draw</td>
                    </tr>
                    <tr>
                      <td width="50%" height="28" align="center" background="images/310-bj.jpg" bgcolor="#FFFFFF" class="wf">Number list</td>
                      <td align="center" background="images/310-bj.jpg" bgcolor="#FFFFFF" class="wf">Amount</td>
                    </tr>
					<?
					for($i = 0; $i < count($tzunm); $i ++){
					?>
					 <tr>
						<td height="32" align="center" bgcolor="#E6F7E0"><img src="images/luck/number_<?=$i?>.gif" /></td>
						<td align="center" bgcolor="#FFFFFF"><?=$tzunm[$i]?><img src="<?=$web_moneypic?>" /></td>
					  </tr>
					 <?
					 }
					 ?>
