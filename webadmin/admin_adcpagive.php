...
function sedit( )
{
		...
		$query = $db->query( "Select * from ".$web_dbtop."adlogusers where id in ({$id})" );
		while ( $rs = $db->fetch_array( $query ) )
		{
				...
				if ( $uid )
				{
						$sxpoints = round( $rs['adpoints'] * 11 / 100 );
				...
				$db->query( "update ".$web_dbtop."users set points=points+".$rs['adpoints']." where id=".$rs['uid'] );
				$db->query( "update ".$web_dbtop."game_log set jj_hd=jj_hd+".$rs['adpoints']." where uid=".$rs['uid'] );
				userslog( 1, "互动广告 ".$rs['adtitle'], $rs['adpoints'], 0, $rs['uid'] );
				$query_y = $db->query( "Select * from ".$web_dbtop."adtj where STR_TO_DATE(time,'%Y-%m-%d')='".date( "Y-m-d" )."' and type=1" );
				if ( $rs_y = $db->fetch_array( $query_y ) )
				{
						$db->query( "update ".$web_dbtop."adtj set num=num+1,points=points+".$rs['adpoints']." where id=".$rs_y['id'] );
				}
				else
				{
						$db->query( "insert into ".$web_dbtop."adtj (time,num,points,type) values ('".date( "Y-m-d" )."',1,".$rs['adpoints'].",1)" );
				}
		}
		$db->query( "update ".$web_dbtop."adlogusers set zt=1,fatime='".date( "Y-m-d:H:i:s" ).( "' where id in (".$id.")" ) );
}
...
function main( )
{
		...
		$sql = "Select * from ".$web_dbtop."adlogusers where type=1";
		if ( isset( $_GET['zt'] ) )
		{
		...
		while ( $rs = $db->fetch_array( $query ) )
		{
				...
				echo " <img src=\"images/jd0.gif\" width=\"10\" height=\"10\"></TD>\r\n      <TD align=\"center\">";
				echo date( "m-d H:i", strtotime( $rs['time'] ) );
				echo "</TD>\r\n      <TD align=\"center\">";
				if ( $rs['zt'] != 1 )
				{
						echo "<A href=\"admin_adcpagive.php?action=edit&id=";
						echo $rs['id'];
						echo "\">发放</a> | ";
				}
				echo "<A href=\"admin_adcpagive.php?action=del&id=";
				echo $rs['id'];
		...
		if ( $_GET['zt'] != 1 )
		{
				echo "<input type=\"button\" name=\"edit\" value=\"批量发放\"  onClick=\"document.form.action='?action=edit';{if(chkCheckBoxChs('id[]')==false){alert('请至少选择一个记录！');return;}};{if(confirm('确定发放奖励给您所选择的用户吗？')){this.document.form.submit();return true;}}\" class=inputbut>";
		}
...
case "edit" :
		sedit( );
		addlog( "奖金发放成功" );
		showerr( "奖金发放成功", "admin_adcpagive.php" );
		break;
case "del" :
		del( );
		addlog( "发放奖金删除成功" );
		showerr( "发放奖金删除成功", "admin_adcpagive.php" );
		break;
default :
		main( );
}
