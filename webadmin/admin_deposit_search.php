...
$sql="Select * from ".$web_dbtop."deposit where uid=".$keyword." Order by id desc";
$query = $db->query( $sql );
?>
...
<BODY>
	<DIV class=bodytitle>
		...
		<DIV class=bodytitletxt>充值记录搜索</DIV>
		<DIV class=bodytitletxt2><a href="admin_deposit_list.php">返回充值记录列表</a></DIV>
	</DIV>
	<!--如果做手动充值，数据库中充值类型改为手动充值-->
	<form method="GET" action="admin_deposit_search.php">
	<div style="width:96%;margin:auto;margin-top:10px;">
		请输入用户id&nbsp;&nbsp;
		<input type="text" name="keywords">
		<input type="submit" value="搜索">
		该页面的搜索表单有些问题，要继续搜索请先返回充值记录列表页面
	</div>
	</form>
