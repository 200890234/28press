...
function del( )
{
				...
				global $web_dbtop;
				$id = implode( ",", $_POST['id'] );
				$query = $db->query( "Select * from ".$web_dbtop."game28_users_tz where NO in ({$id})" );
				while ( $rs = $db->fetch_array( $query ) )
				{
								$db->query( "update ".$web_dbtop."users set points=points-".$rs['hdpoints']."+".$rs['points']." where id=".$rs['uid'] );
								$db->query( "update ".$web_dbtop."game_log set game28_hd=game28_hd-".$rs['hdpoints']."+".$rs['points']." where uid=".$rs['uid'] );
				}
				$db->query( "delete from ".$web_dbtop."game28_users_tz where NO in ({$id})" );
				$db->query( "delete from ".$web_dbtop."game28 where id in ({$id})" );
}
...
function main( )
{
				...
				$query = $db->query( "Select * from ".$web_dbtop."game28 Order by id desc" );
				if ( $db->fetch_array( $query ) )
				{
								$intnum = $db->num_rows( $query );
				}
				$query = $db->query( "Select *,(select sum(hdpoints) from ".$web_dbtop."game28_users_tz where NO=a.id) as hdpoints from {$web_dbtop}game28 as a Order by id desc limit {$rsnum},{$intpage}" );
				while ( $rs = $db->fetch_array( $query ) )
				{
								$kgh = explode( "|", $rs['kgjg'] );
								...
								if ( $rs['kj'] == 1 )
								{
												echo substr( $kgh[0], -1 )." + ".substr( $kgh[1], -1 )." + ".substr( $kgh[2], -1 )." = ".$kgh[3];
								}
								else
								{
												echo "未开奖";
								...
								echo "','";
								echo $rs['id'];
								echo "期 投注详情','admin_game28_tj_c.php?action=tz&NO=";
								echo $rs['id'];
								...
								if ( $rs['kj'] != 1 )
								{
												...
												echo "','";
												echo $rs['id'];
												echo "期 手动干预开奖','admin_game28_tj_c.php?action=sd&NO=";
												echo $rs['id'];
