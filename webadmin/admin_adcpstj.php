...
function del( )
{
		global $db;
		global $web_dbtop;
		$db->query( "delete from ".$web_dbtop."adtj where STR_TO_DATE(time,'%Y-%m-%d')<='".date( "Y-" ).( date( "m" ) - 2 ).date( "-d" )."' and type=2" );
}
...
function main( )
{
		...
		$sql = "Select * from ".$web_dbtop."adtj where type=2";
