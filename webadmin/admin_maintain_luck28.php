...
if($action=="docancel"){
	...
	$sql="update ".$web_dbtop."game28 set iscancel='yes' where id in (".$ids.")";
	$db->query($sql);
	echo "<script type=\"text/javascript\">alert(\"取消成功\");</script>";
}
$sql="Select * from ".$web_dbtop."game28 where kj=0 Order by id desc limit 5";
$query = $db->query( $sql );
?>
...
<BODY>
	<DIV class=bodytitle>
		...
		<DIV class=bodytitletxt>幸运28维护</DIV>
