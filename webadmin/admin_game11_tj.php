...
function del( )
{
				...
				global $web_dbtop;
				$id = implode( ",", $_POST['id'] );
				$query = $db->query( "Select * from ".$web_dbtop."game11_users_tz where NO in ({$id})" );
				while ( $rs = $db->fetch_array( $query ) )
				{
								$db->query( "update ".$web_dbtop."users set points=points-".$rs['hdpoints']."+".$rs['points']." where id=".$rs['uid'] );
								$db->query( "update ".$web_dbtop."game_log set game11_hd=game11_hd-".$rs['hdpoints']."+".$rs['points']." where uid=".$rs['uid'] );
				}
				$db->query( "delete from ".$web_dbtop."game11_users_tz where NO in ({$id})" );
				$db->query( "delete from ".$web_dbtop."game11 where id in ({$id})" );
}
...
function main( )
{
				...
				$query = $db->query( "Select * from ".$web_dbtop."game11 Order by id desc" );
				if ( $db->fetch_array( $query ) )
				{
								$intnum = $db->num_rows( $query );
				}
				$query = $db->query( "Select *,(select sum(hdpoints) from ".$web_dbtop."game11_users_tz where NO=a.id) as hdpoints from {$web_dbtop}game11 as a Order by id desc limit {$rsnum},{$intpage}" );
				while ( $rs = $db->fetch_array( $query ) )
				{
								$kgh = explode( "|", $rs['kgjg'] );
								...
								echo "','";
								echo $rs['id'];
								echo "期 投注详情','admin_game11_tj_c.php?action=tz&NO=";
								echo $rs['id'];
								...
								if ( $rs['kj'] != 1 )
								{
												...
												echo "','";
												echo $rs['id'];
												echo "期 手动干预开奖','admin_game11_tj_c.php?action=sd&NO=";
												echo $rs['id'];
												echo "',300,140,0);return false\"><img src=\"images/AutoTextGallery.gif\" alt=\"手动干预开奖\"></a>";
								}
								echo "</TD>\r\n    </TR>\r\n    ";
				}
