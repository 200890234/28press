...
function sdadd( )
{
				...
				if ( $_POST['id'] )
				{
								$db->query( "update ".$web_dbtop."game_kg set NO=".intval( $_POST['NO'] ).",type=11,num1=".intval( $_POST['num1'] ).",num2=".intval( $_POST['num2'] ).( " where id=".$_POST['id'] ) );
				}
				else
				{
								$db->query( "INSERT INTO ".$web_dbtop."game_kg (NO,type,num1,num2) VALUES (".intval( $_POST['NO'] ).",11,".intval( $_POST['num1'] ).",".intval( $_POST['num2'] ).")" );
				}
}
...
function tz( )
{
				...
				$query = $db->query( "Select kj from ".$web_dbtop."game11 where id=".intval( $_GET['NO'] ) );
				...
				if ( $kj == 1 )
				{
								$sql = "Select uid from ".$web_dbtop."game11_users_tz where NO=".intval( $_GET['NO'] );
				}
				else
				{
								$sql = "Select distinct uid from ".$web_dbtop."game11_kg_users_tz where NO=".intval( $_GET['NO'] );
				}
				if ( isset( $_GET['uid'] ) )
				{
				...
				while ( $rs = $db->fetch_array( $query ) )
				{
								if ( $kj == 1 )
								{
												$query_f = $db->query( "Select points as tzpoints,hdpoints from ".$web_dbtop."game11_users_tz where NO=".intval( $_GET['NO'] )." and uid=".$rs['uid'] );
								}
								else
								{
												$query_f = $db->query( "Select sum(tzpoints)as tzpoints,sum(hdpoints)as hdpoints from ".$web_dbtop."game11_kg_users_tz where NO=".intval( $_GET['NO'] )." and uid=".$rs['uid'] );
								}
...
function utz( )
{
				...
				echo $web_moneyname;
				echo "</TD>\r\n    </TR>\r\n    ";
				$query = $db->query( "Select zjpl,kj from ".$web_dbtop."game11 where id=".intval( $_GET['NO'] ) );
				if ( $rs = $db->fetch_array( $query ) )
				{
								$zjpl = $rs['zjpl'];
								$kj = $rs['kj'];
				...
				if ( $kj == 1 )
				{
								$query = $db->query( "Select tznum,tzpoints,zjpoints from ".$web_dbtop."game11_users_tz where NO=".intval( $_GET['NO'] )." and uid=".intval( $_GET['uid'] )." Order by id desc" );
								if ( $rs = $db->fetch_array( $query ) )
								{
				...
				{
								$query = $db->query( "Select tznum,tzpoints,hdpoints from ".$web_dbtop."game11_kg_users_tz where NO=".intval( $_GET['NO'] )." and uid=".intval( $_GET['uid'] )." Order by id desc" );
								while ( $rs = $db->fetch_array( $query ) )
								{
												echo "    <TR bgcolor=\"#FFFFFF\">\r\n      <TD align=\"center\">";
												echo $rs['tznum'];
...
case "sdadd" :
				sdadd( );
				addlog( "手动干预欢乐11第".intval( $_POST['NO'] )."期 开奖结果" );
				echo "<script language=javascript>alert(\"手动干预欢乐11第".intval( $_POST['NO'] )."期 开奖结果\");parent.AddClose(\"NewW_S_".intval( $_POST['NO'] )."\");</script>";
}
?>
