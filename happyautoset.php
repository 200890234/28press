...
$query=$db->query("Select id from {$web_dbtop}game16 where kj=0 Order by id asc");
...
if($rs=$db->fetch_array($query)){
	echo "<script language=javascript>location.href='happyautodetail.php';</script>";
	exit;
}

if($_GET["act"] =="autostart"){
	if(!$_POST["tbStartNO"] || !$_POST["tbEndNO"]){
		echo "<script language=javascript>alert('all fields are required');location.href='happyautoset.php';</script>";
		exit;
	}
	if(!is_numeric($_POST["tbStartNO"]) || !is_numeric($_POST["tbEndNO"]) || !is_numeric($_POST["tbMinG"]) || !is_numeric($_POST["tbMaxG"])){
		echo "<script language=javascript>alert('invalid string format');location.href='happyautoset.php';</script>";
		exit;
	}
	if(!$_POST["tbID"]){
		echo "<script language=javascript>alert(\"sorry, you haven't create any pattern yet.\");location.href='happyautoset.php';</script>";
		exit;
	}
	if($_POST["tbStartNO"]<($kgqh+1)){
		echo "<script language=javascript>alert(\"sorry,the start draw must be greater than the current one.\");location.href='happyautoset.php';</script>";
		exit;
	}
	if($_POST["tbStartNO"]>$_POST["tbEndNO"]){
		echo "<script language=javascript>alert(\"make sure the end draw is greater than the start draw\");location.href='happyautoset.php';</script>";
		exit;
	}
	if(($_POST["tbEndNO"]-$_POST["tbStartNO"])>99999){
		echo "<script language=javascript>alert(\"sorry, the total draws is no more than 99999.\");location.href='happyautoset.php';</script>";
		exit;
	}
	if($_POST["tbMinG"]<0){
		echo "<script language=javascript>alert('sorry,please check your minimum balance');location.href='happyautoset.php';</script>";
		exit;
	}
	if($_POST["tbMaxG"]<0){
		echo "<script language=javascript>alert('sorry,please check your maximum balance');location.href='happyautoset.php';</script>";
		exit;
	}
	if($_POST["tbMaxG"]-$_POST["tbMinG"]<0){
		echo "<script language=javascript>alert(\"maximum balance should be greater that minimum balance\");location.href='happyautoset.php';</script>";
		exit;
	}
	$db->query("INSERT INTO {$web_dbtop}game16_auto (startNO,endNO,minG,maxG,autoid,uid) VALUES (".intval($_POST["tbStartNO"]).",".intval($_POST["tbEndNO"]).",".intval($_POST["tbMinG"]).",".intval($_POST["tbMaxG"]).",".intval($_POST["tbID"]).",".intval($_COOKIE["usersid"]).")");
	for($i = 0; $i < count($_POST["sID"]); $i ++){
		$db->query("update {$web_dbtop}game16_auto_tz set winid=".intval($_POST["sWinModel"][$i]).",lossid=".intval($_POST["sLossModel"][$i])." where id =".intval($_POST["sID"][$i]));
	}
	echo "<script language=javascript>alert('autopress setting success');location.href='happyautodetail.php';</script>";
	exit;
}
?>
			...
			<DIV class="webgame-info-tabs">
			  ...
			  <form action="?act=autostart" method="post" onsubmit="return ChkSetAuto()">
                <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
                  <tr>
                    <td bgcolor="#9F4F18"><table width="100%" border="0" cellspacing="1" cellpadding="4">
                        <tr>
					...
					?>
                              <option label="<?=$rs["tzname"]?>" value="<?=$rs["id"]?>">
                              <?=$rs["tzname"]?>
                              </option>
                              <?
					}
					?>
							...
							<a href="happymodel.php" style="color:green;"> create new pattern</a>
						  </td>
                        </tr>
                        <tr>
                        ...
                        <tr>
                          ...
                          <td bgcolor="#FFFFFF"><input type="text" name="tbEndNO" id="tbEndNO" value="<?=$kgqh+99999?>" /> must be than <?=$kgqh+99999?> </td>
                        </tr>
                        <tr>
                          <td height="28" align="right" bgcolor="#FFFFFF">Minimum balance <?=$web_moneyname?>：</td>
                          <td bgcolor="#FFFFFF"><input type="text" name="tbMinG" id="tbMinG" value="0" /> terminate autopress when your balance <?=$web_moneyname?> is less than it</td>
                        </tr>
                        <tr>
                        ...
                        <tr>
                          <td height="85" colspan="2" align="center" bgcolor="#FFFFFF" class="rf"><table width="100%" border="0" cellpadding="0" cellspacing="3">
                              <tr>
                              ...
                              <?
						$i=0;
						$query_f=$db->query("Select * from {$web_dbtop}game16_auto_tz where uid=".intval($_COOKIE["usersid"])." Order by tzid asc");
						while($rs_f=$db->fetch_array($query_f)){
						?>
                              <tr>
                                <td align="center"><input type="hidden" name="sID[]" id="sID<?=$i;?>" value="<?=$rs_f["id"]?>" />
                                    <?=$rs_f["tzname"]?></td>
						...
						?>
                              ...
                              <br />
                              <input name="image" type="image" id="btnSubmit" src="images/happy16_01.gif" width="115" height="29" border="0" /><div class="clear" style="height:20px;"></div><!--只用来分隔-->
                &nbsp;<!--<img src="images/happy16_02.gif" width="115" height="29" border="0" class="pointer" onclick="javascript:history.back()" />--></td>
                        </tr>
                        <!--<tr>
                          <td height="28" colspan="2" align="left" valign="top" bgcolor="#FFFFFF"><p >Users guide：<br />
			  ...
			  <script type="text/javascript">
				...
				/*function ChkSetAuto(){
					...
					if(ChkInt(endLuckNO,1,10)==false || endLuckNO > s+2999 || endLuckNO <= starLuckNO){
						alert("结束投注期数格式错误！");
						return false;
					}
					if(ChkInt(minMoney,1,10,"[0-9]+")==false){
						alert("<?=$web_moneyname?>最小值格式错误！");
						return false;
					}
